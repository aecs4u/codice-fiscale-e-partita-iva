...
from codice_fiscale.data import get_indexed_data
...
def encode_birthdate(
    ...
    """
    if not birthdate:
        raise ValueError("[codice_fiscale] 'birthdate' argument cant be None")
    date = _get_date(birthdate)
    if not date:
        raise ValueError("[codice_fiscale] 'date' argument cant be None")

    if not gender:
        raise ValueError("[codice_fiscale] 'gender' argument cant be None")
    gender_code = gender.upper()
    if gender_code not in ("M", "F"):
        raise ValueError("[codice_fiscale] 'gender' argument must be 'M' or 'F'")
...
def encode_birthplace(
    ...
    """
    if not birthplace:
        raise ValueError("[codice_fiscale] 'birthplace' argument cant be None")
...
def encode_cin(code: str) -> str:
    ...
    """
    if not code:
        raise ValueError("[codice_fiscale] 'code' argument cant be None")

    code_len = len(code)
    if code_len not in [15, 16]:
        raise ValueError(
            f"[codice_fiscale] 'code' length must be 15 or 16, not: {code_len}"
        )
...
def decode_raw(code: str) -> dict[str, str]:
    ...
    if not match:
        raise ValueError(f"[codice_fiscale] invalid syntax: {code}")
...
def decode(code: str) -> dict[str, Any]:
    ...
    if not birthdate:
        raise ValueError(f"[codice_fiscale] invalid date: {birthdate_str}")
    ...
    if not birthplace:
        raise ValueError(
            "[codice_fiscale] wrong birthplace code: "
